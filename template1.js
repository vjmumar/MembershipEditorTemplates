    ...
    class CourseTemplate {
                ...
                this.utils.waitForElement(".product-container", async ($container) => {
                    ...
                    const bannerButtonLinkAndText = await (async () => {
                        ...
                        if (userProductProgress?.completedPosts >= 1) {
                            const lastCompletedPost = allPosts.find((e) => e.id === completedPosts.slice[0]?.postId);
                            text = "Resume Course";
                            console.log(lastCompletedPost, completedPosts)
                            nextPost = allPosts[lastCompletedPost.index + 1];
                        } else {
                            nextPost = allPosts[0];
                        }
        ...
        // Utils module
        utils = {
            waitForElement: (elementSelector = "", cb = (element) => null, cbInvokationDelay = 1000) => {

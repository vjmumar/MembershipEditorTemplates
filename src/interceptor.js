   ...
   data = {
      ...
      getLoaderHTML: async () => {
         try {
            ...
            const savedTemplates = await (async () => {
               ...
               try {
                  ...
                  if (savedTemplateFilesInfo?.url) {
                     const templates = await fetch(savedTemplateFilesInfo.url, { priority: "high" })
                        .then((e) => e.text())
                        .then((e) =>
                           e
                              .replace(/^var\s+s\s*=\s*/, "")
            ...
            const activeSavedTemplates = savedTemplates.find((template) => template.isActive);
            ...
            const activeDefaultTemplate = await (async () => {
               ...
               const defaultTemplateUrl = (activeClientTemplate || clientTemplates?.[0])?.scriptLink || "";
               ...
               return await fetch(defaultTemplateUrl)
                  ...
                  .then((e) => {
                     ...
                     const getArrayValue = (key) =>
                        safeJsonParse(
                           config?.match(new RegExp(`${key}:\\s*(\\[[\\s\\S]*?\\])`))?.[1] || "[]",
                           [],
                        );
                     const toJson = (value = "") =>
                        value
            ...
            const loadingConfig =
               activeSavedTemplates?.global?.find((item) => item?.name === "Loading Overlay") ||
               activeDefaultTemplate?.global?.[0];
            const loadingHTML = (() => {
               const getCustomization = (selector = "") =>
                  loadingConfig?.customizations?.find((item) => item?.elementSelector === selector);
               const getCss = (customization = {}, property = "") =>
                  customization?.cssCustomizations?.find((item) => item?.property === property);
               const getElement = (customization = {}, target = "") =>
                  customization?.elementCustomizations?.find((item) => item?.target === target);
               const cssDecl = (property = "", c = {}) =>
                  c?.value ? `${property}: ${c.value}${c.important ? " !important" : ""};` : "";
               ...
               const containerStyles = cssDecl("background-color", getCss(containerConfig, "background-color"));
               const logoStyles = [
   ...
   async fetch(request) {
      ...
      if (contentType.includes("text/html")) {
         ...
         if (proxy.isProduct) {
            ...
            loader = auth?.tokenId ? await proxy.data.getLoaderHTML() : proxy.defaultLoaderHTML;
            script = `
                  <script>
